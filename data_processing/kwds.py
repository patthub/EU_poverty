...
Original file is located at
    https://colab.research.google.com/drive/1Oqt2vbxwYjHzPKfVobgujFjF-OkD7Gqu
"""
...
# Wyświetlenie pierwszych kilku wierszy tabeli, aby zrozumieć strukturę danych
df.head()

# Krok 1: Normalizacja kolumny `keyword`
...
# Wyświetlenie rozmiaru danych przed i po usunięciu duplikatów
...
# Definiowanie funkcji do grupowania słów kluczowych
...
# Stosowanie funkcji grupowania do kolumny 'normalized_keyword' i tworzenie nowej kolumny 'keyword_group'
df_deduplicated['keyword_group'] = df_deduplicated['normalized_keyword'].apply(group_keywords)

# Wyświetlenie pierwszych kilku wierszy z nową kolumną 'keyword_group'
df_deduplicated[['normalized_keyword', 'keyword_group']].head()

# Mapowanie dla grupowania podobnych słów kluczowych
keyword_mapping = {
...
# Aktualizacja słów kluczowych zgodnie z zasadami grupowania
df_deduplicated['grouped_keyword'] = df_deduplicated['normalized_keyword'].replace(keyword_mapping)

# Sprawdzenie unikalnych wartości w nowej kolumnie 'grouped_keyword'
unique_grouped_keywords = sorted(df_deduplicated['grouped_keyword'].unique())
unique_grouped_keywords

df_deduplicated

# Wczytanie nowej tabeli z przypisaniami słów kluczowych
new_keywords_path = '/content/new_kwds - main(3).csv'
new_keywords_df = pd.read_csv(new_keywords_path)

# Wyświetlenie pierwszych kilku wierszy z nowej tabeli
new_keywords_df.head()

# Usunięcie wierszy z pustymi wartościami w nowej tabeli słów kluczowych
new_keywords_df_cleaned = new_keywords_df.dropna()

# Przygotowanie słownika do mapowania oryginalnych słów kluczowych na główne słowa kluczowe
...
# Aktualizacja słów kluczowych w pierwotnej tabeli danych
...
# Wyświetlenie pierwszych kilku wierszy zaktualizowanej tabeli danych
df_deduplicated[['grouped_keyword', 'main_keyword']].head()

# Ponowne zdefiniowanie mapowania, uwzględniając potencjalne różnice w pisowni i formatowaniu
# Normalizacja słów kluczowych w obu tabelach do porównania
df_deduplicated['normalized_grouped_keyword'] = df_deduplicated['grouped_keyword'].str.strip().str.lower()
new_keywords_df_cleaned['normalized_keyword'] = new_keywords_df_cleaned['keyword'].str.strip().str.lower()

# Ponowne stworzenie słownika mapowania z uwzględnieniem normalizacji
...
# Aktualizacja słów kluczowych w pierwotnej tabeli danych z uwzględnieniem normalizacji
...
# Wyświetlenie pierwszych kilku wierszy z poprawionymi przypisaniami głównych słów kluczowych
df_deduplicated[['grouped_keyword', 'main_keyword_corrected']].head()
